...
class LazyJavaPackageFragment(
    ...
    private val scope by c.storageManager.createLazyValue {
        val partScopes = binaryClasses.values.mapNotNull { partClass ->
            c.components.deserializedDescriptorResolver.createKotlinPackagePartScope(this, partClass)
        }
        if (partScopes.isEmpty()) javaScope
        else ChainedMemberScope("Chained Java + deserialized Kotlin scope", partScopes + javaScope) // TODO: first or last?
    }
