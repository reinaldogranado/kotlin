...
class IneffectiveStatementElimination(private val root: JsStatement) {
    ...
    private fun analyze() {
        localVars += collectDefinedNames(root)
    }
    ...
    private fun replace(expression: JsExpression): List<JsExpression> {
        return when (expression) {
            ...
            is JsConditional -> {
                if (replace(expression.thenExpression).isEmpty() && replace(expression.elseExpression).isEmpty()) {
                    replace(expression.testExpression)
                }
                else {
                    listOf(expression)
                }
